...
export function toBookProject(project: ResearchProject, base: BookProject): BookProject {
  const book = project.generatedBook;
  return { ...base, id: crypto.randomUUID(), updatedAt: new Date().toISOString(), status: "draft", title: book.title, subtitle: book.subtitle, series: book.series || "", author: book.author || "Publisher Name", sections: book.sections.map((section) => ({ id: section.id, name: section.name, description: section.description, puzzles: section.puzzles.map((puzzle) => ({ id: puzzle.id, title: puzzle.title, blurb: puzzle.blurb, words: puzzle.words.map((word) => word.display) })) })) };
}
